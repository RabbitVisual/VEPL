...
<div x-data="sermonBuilder()" class="mx-auto max-w-7xl px-3 py-4 sm:px-6">
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Estúdio de Sermão</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400">Estruture uma mensagem completa com base bíblica, clareza pastoral e aplicação prática.</p>
        </div>
        ...
        <section class="space-y-4 lg:col-span-3">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <input name="theme" value="{{ old('theme') }}" placeholder="Tema central" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <input type="date" name="sermon_date" value="{{ old('sermon_date') }}" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                    <select name="sermon_structure_type" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                        ...
                        <option value="expositivo" @selected(old('sermon_structure_type') === 'expositivo')>Expositivo</option>
                        <option value="temático" @selected(old('sermon_structure_type') === 'temático')>Temático</option>
                        <option value="textual" @selected(old('sermon_structure_type') === 'textual')>Textual</option>
                    </select>
                </div>
                <label class="mt-3 inline-flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                    <input type="checkbox" name="is_collaborative" value="1" @checked(old('is_collaborative')) class="rounded border-slate-300 text-amber-600 focus:ring-amber-600">
                    ...
                    <div class="grid grid-cols-2 gap-2 md:grid-cols-3">
                        @foreach($tags as $tag)
                            <label class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">
                                <input type="checkbox" name="tags[]" value="{{ $tag->id }}" @checked(collect(old('tags', []))->contains($tag->id)) class="rounded border-slate-300 text-amber-600 focus:ring-amber-600">
                                {{ $tag->name }}
                            </label>
                        @endforeach
                    </div>
