...
@section('title', 'Novo Sermão Expositivo')

@section('content')
<div x-data="sermonBuilder()" class="mx-auto max-w-7xl px-3 py-4 sm:px-6">
    <div class="mb-4 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Sermon Builder</h1>
            <p class="text-sm text-slate-600 dark:text-slate-400">Estúdio de Pregação Expositiva com Bíblia integrada</p>
        </div>
        <a href="{{ route('memberpanel.sermons.index') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800">Voltar</a>
    </div>

    <form action="{{ route('memberpanel.sermons.store') }}" method="POST" class="grid grid-cols-1 gap-4 lg:grid-cols-5">
        @csrf
        <section class="space-y-4 lg:col-span-3">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <input name="title" value="{{ old('title') }}" required placeholder="Título do sermão" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                    <input name="theme" value="{{ old('theme') }}" placeholder="Tema" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                    <input name="biblical_text_base" value="{{ old('biblical_text_base') }}" required placeholder="Texto base: Romanos 8:1-11" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 md:col-span-2">
                    <input name="subtitle" value="{{ old('subtitle') }}" placeholder="Subtítulo" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 md:col-span-2">
                    <textarea name="description" rows="2" placeholder="Resumo pastoral" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 md:col-span-2">{{ old('description') }}</textarea>
                </div>
            </div>

            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                <label class="mb-1 block text-sm font-semibold text-slate-700 dark:text-slate-200">Contexto histórico-gramatical</label>
                <x-rich-editor name="historical_context" :value="old('historical_context')" />
                <label class="mb-1 mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">Proposição central</label>
                <x-rich-editor name="central_proposition" :value="old('central_proposition')" />
                <label class="mb-1 mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">Introdução</label>
                <x-rich-editor name="introduction" :value="old('introduction')" />
                <label class="mb-1 mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">Corpo do esboço</label>
                <x-rich-editor name="body_outline" :value="old('body_outline')" />
                <label class="mb-1 mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">Aplicação prática</label>
                <x-rich-editor name="practical_application" :value="old('practical_application')" />
                <label class="mb-1 mt-4 block text-sm font-semibold text-slate-700 dark:text-slate-200">Conclusão</label>
                <x-rich-editor name="conclusion" :value="old('conclusion')" />
            ...
            @include('sermons::components.bible-reference-picker', ['versions' => $bibleVersions, 'references' => old('bible_references', [])])
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <select name="sermon_series_id" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                        ...
                        @foreach($series as $serie)
                            <option value="{{ $serie->id }}" @selected(old('sermon_series_id') == $serie->id)>{{ $serie->title }}</option>
                        @endforeach
                    </select>
                    ...
                    <select name="visibility" class="rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                        <option value="members" @selected(old('visibility', 'members') === 'members')>Membros</option>
        ...
        <aside class="lg:col-span-2">
            <div class="sticky top-4 space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                <h3 class="text-sm font-semibold text-slate-900 dark:text-slate-100">Sidebar Bíblica</h3>
                <input x-model="bibleQuery" @keydown.enter.prevent="searchBible()" placeholder="Buscar texto bíblico (ex: Jo 3:16)" class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                <button type="button" @click="searchBible()" class="w-full rounded-lg bg-slate-800 px-3 py-2 text-sm text-white hover:bg-slate-700 dark:bg-slate-200 dark:text-slate-900">Buscar</button>
                <input x-model="strongQuery" @keydown.enter.prevent="searchStrong()" placeholder="Strong (ex: G26)" class="w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                <button type="button" @click="searchStrong()" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-100 dark:hover:bg-slate-800">Consultar Strong</button>
...
function sermonBuilder() {
    return {
        ...
        references: [],
        init() {
            this.references = (@json(old('bible_references', [])) || []).map(this.decorateReference);
        },
        decorateReference(reference = {}) {
            return { uid: crypto.randomUUID(), bible_version_id: reference.bible_version_id || '', type: reference.type || 'main', book_id: reference.book_id || '', chapter_id: reference.chapter_id || '', verse_start_id: reference.verse_start_id || '', verse_end_id: reference.verse_end_id || '', context: reference.context || '', books: [], chapters: [], verses: [] };
        },
        async searchBible() {
