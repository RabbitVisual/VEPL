        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Entradas</h2>
                    <div class="text-sm text-gray-500 dark:text-gray-400">Total: {{ $lexicons->total() }}</div>
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($lexicons as $entry)
                            <tr class="align-top">
                                <td class="px-4 py-3 font-mono text-blue-700 dark:text-blue-300">{{ $entry->number }}</td>
                                <td class="px-4 py-3">{{ $entry->lang }}</td>
                                <td class="px-4 py-3">{{ $entry->lemma }}</td>
                                <td class="px-4 py-3">{{ $entry->pronounce }}</td>
                                <td class="px-4 py-3 text-gray-700 dark:text-gray-200">
                                    {{ \Illuminate\Support\Str::limit((string) $entry->description_pt, 70) }}
                                </td>
                                <td class="px-4 py-3">
                                    @if($entry->is_reviewed)
                                        <span class="inline-flex px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">Sim</span>
                                    @else
                                        <span class="inline-flex px-3 py-1 rounded-full text-xs font-bold bg-gray-100 text-gray-600 dark:bg-gray-900/30 dark:text-gray-300">Não</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3">
